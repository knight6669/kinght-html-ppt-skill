...
- 讲义视图由 `CURRENT`、`NEXT`、逐字稿和计时器组成，打开、刷新和调整窗口后都会自动恢复稳定排版。
- 当前页和下一页预览铺满容器，适合边讲边看；在当前页区域滚动鼠标滚轮即可同步翻页。
- 逐字稿可直接编辑并自动保存，`Ctrl+B` 会变成主题强调色，取消加粗后恢复正文色。
- `O` 页面总览与 `E` 页面导览都采用主题 token，缩略图、编号、边框和背景会随主题自然协调。
